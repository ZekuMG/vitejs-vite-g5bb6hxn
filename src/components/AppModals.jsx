    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[100] p-4 backdrop-blur-[2px] animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden scale-100 transform transition-all">
        ...
        <div className="p-6 flex flex-col items-center text-center">
          <div className={`w-16 h-16 rounded-full bg-slate-50 border-4 ${borderClass} flex items-center justify-center mb-4 shadow-sm`}>
            {icon}
          </div>
          ...
          <button 
            onClick={onClose}
            className={`w-full py-2.5 rounded-lg text-white font-bold text-sm shadow-md transition-all active:scale-95 ${btnClass}`}
          >
            Entendido
          </button>
        </div>
...
export const ClientSelectionModal = ({ isOpen, onClose, onSelectClient, clients = [], addClient, onCancelFlow }) => {
  ...
  const handleCreate = (e) => {
    ...
    if (newMemberData.name) {
      ...
      if (newMember) {
        onSelectClient(newMember, 'earn'); // Auto-seleccionar para sumar puntos
        onClose();
      }
    }
  };
          ...
          {step === 'check-want-join' && (
            <div className="text-center">
              ...
              <div className="space-y-3">
                ...
                <button 
                  onClick={() => {
                    if (onCancelFlow) onCancelFlow(); // Proceder como invitado
                    onClose();
                  }}
...
// ==========================================
// MODALES DE NEGOCIO (Continuación)
// ==========================================
      ...
      <div className="bg-white rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden w-full max-w-sm">
        ...
        <div className="overflow-y-auto p-6 bg-slate-200 flex justify-center">
          ...
          >
            ...
            {/* Fidelización (Lógica similar a TicketPrintLayout) */}
            {transaction.client ? (
              <>
