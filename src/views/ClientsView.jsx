...
export default function ClientsView({ onViewTicket }) {
  const { members, addMember, updateMember, deleteMember } = useClients();
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex-1 overflow-y-auto">
        <table className="w-full text-left">
          <thead className="bg-gray-50/50 border-b border-gray-100 sticky top-0 z-10 backdrop-blur-sm">
            <tr>
              <th className="p-4 font-bold text-gray-500 text-xs uppercase tracking-wider">Socio</th>
              <th className="p-4 font-bold text-gray-500 text-xs uppercase tracking-wider">N° Socio</th>
              <th className="p-4 font-bold text-gray-500 text-xs uppercase tracking-wider">Contacto</th>
              <th className="p-4 font-bold text-gray-500 text-xs uppercase tracking-wider">Puntos</th>
                ...
                <tr key={member.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 text-blue-700 flex items-center justify-center font-bold shadow-sm text-sm border border-white">
                        {member.name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p className="font-bold text-gray-900">{member.name}</p>
                        {member.extraInfo && <p className="text-xs text-gray-400 max-w-[150px] truncate">{member.extraInfo}</p>}
                      </div>
                    </div>
                  </td>
                ...
                {selectedMember.history && selectedMember.history.length > 0 ? (
                  selectedMember.history.map((mov) => (
                    <div key={mov.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all">
                      ...
                      <div className="flex justify-between items-start mb-3 border-b border-gray-50 pb-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-400 font-medium">
                            {new Date(mov.date).toLocaleDateString()} • {new Date(mov.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} hs
                          </p>
                        </div>
                        {mov.orderId && (
                      ...
                      {/* Detalles: Monto y Evolución de Puntos */}
                      <div className="flex items-center justify-between">
                        ...
                        {/* Evolución de Puntos (Antes -> Después) */}
                      ...
                      {/* Botón Ver Ticket (Solo si hay orderId y se pasó la función) */}
                      {mov.orderId && onViewTicket && (
                        <button 
                          onClick={() => {
                            // Simulamos estructura de transacción mínima para el visualizador
                            onViewTicket({ id: mov.orderId });
                          }}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  className="w-full rounded-lg border border-gray-300 p-2.5 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none font-medium"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    className="w-full rounded-lg border border-gray-300 p-2.5 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none font-mono text-sm"
              ...
              <div>
                ...
                <input
                  type="email"
                  className="w-full rounded-lg border border-gray-300 p-2.5 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none text-sm"
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Notas / Extra</label>
                <textarea
                  rows="2"
                  className="w-full rounded-lg border border-gray-300 p-2.5 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none text-sm resize-none"
              ...
              <div className="pt-2 flex gap-3">
                ...
                >
                  <Save size={18} />
                  {modalMode === 'create' ? 'Registrar Socio' : 'Guardar Cambios'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 text-center">
              ...
              <p className="text-gray-500 text-sm mb-6">
                Estás a punto de eliminar a <span className="font-bold text-gray-800">{memberToDelete?.name}</span>. 
                <br/>Esta acción no se puede deshacer y se perderán sus puntos e historial.
              </p>
